    ...
    <div class="col-auto ms-auto d-print-none">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb bg-transparent p-0 mt-1 mb-0">
                ...
                <li class="breadcrumb-item"><a href="#">Rak</a></li>
                            ...
                            <tr>
                                ...
                                <td width="15%">
                                    ...
                                    <a href="javascript:void(0)" class="btn btn-icon btnEdit btn-warning text-white"
                                        data-id="{{ $rack->rack_id }}" data-toggle="tooltip" data-placement="top"
                                        title="Ubah">
                                        <i data-feather="edit" width="16" height="16"></i>
                                    </a>
                                    <a href="javascript:void(0)" class="btn btn-icon btn-danger text-white btnDelete"
                                        data-url="{{ url('rack/delete/'. $rack->rack_id) }}" data-toggle="tooltip"
...
<div class="modal fade addModal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ url('rack/store') }}" method="POST" id="addForm">
                @csrf
                        ...
                        <div class="row">
                            <div class="col-md-12 mb-3">
                                <div class="form-group">
                                    <label class="form-label">Rak Name<span class="text-danger">*</span></label>
                                    <input type="text" class="form-control" name="rack_name" id="rack_name"
                                        placeholder="Masukan Rak Name" value="{{ old('rack_name') }}">
                                </div>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
    $('.btnAdd').click(function () {
        $('#rack_name').val('');
        $('.addModal form').attr('action', "{{ url('rack/store') }}");
        $('.addModal .modal-title').text('Tambah Rak');
        $('.addModal').modal('show');
    });
    ...
    $('.btnEdit').click(function () {
        ...
        var url = "{{ url('rack/getdata') }}";

        $('.addModal form').attr('action', "{{ url('rack/update') }}" + '/' + id);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.status == 1) {
                    $('#rack_name').val(data.result.rack_name);
                    $('.addModal .modal-title').text('Ubah Rak');
                    $('.addModal').modal('show');
                }
    ...
    $("#addForm").validate({
        ...
        messages: {
            rack_name: "Rak Name Tidak Boleh Kosong",
        },
        errorElement: "em",
