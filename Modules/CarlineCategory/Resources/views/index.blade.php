    ...
    <div class="col-auto ms-auto d-print-none">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb bg-transparent p-0 mt-1 mb-0">
                ...
                <li class="breadcrumb-item"><a href="#">Carline</a></li>
                <li class="breadcrumb-item active" aria-current="page">Carline</li>
            </ol>
        </nav>
    </div>
</div>
                    ...
                    <table id="table-data" class="table card-table table-vcenter text-nowrap table-data">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td width="80%">{{ $carlinecategory->carline_category_name }}</td>
                                <td width="15%">
                                    <a href="javascript:void(0)" class="btn btn-icon btnEdit btn-warning text-white"
                                        data-id="{{ $carlinecategory->carline_category_id }}" data-toggle="tooltip" data-placement="top"
                                        title="Ubah">
                                        <i data-feather="edit" width="16" height="16"></i>
                                    </a>
                                    <a href="javascript:void(0)" class="btn btn-icon btn-danger text-white btnDeleteData"
                                        data-url="{{ url('carlinecategory/delete/'.$carlinecategory->carline_category_id)}}" data-toggle="tooltip"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Tambah Carline</h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <form action="{{ url('carlinecategory/store') }}" method="POST" id="addForm">
                @csrf
...
<script type="text/javascript">
    $('.btnAdd').click(function () {
        $('#module_name').val('');
        $('.addModal form').attr('action', "{{ url('carlinecategory/store') }}");
        $('.addModal .modal-title').text('Tambah Carline');
        $('.addModal').modal('show');
    });
    ...
    $('.btnEdit').click(function () {
        ...
        var url = "{{ url('carlinecategory/getdata') }}";


        $('.addModal form').attr('action', "{{ url('carlinecategory/update') }}" + '/' + id);
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.status == 1) {
                    ...
                    $('#carline_category_name').val(data.result.carline_category_name);
                    $('.addModal .modal-title').text('Ubah Carline');
                    $('.addModal').modal('show');
    ...
    $("#addForm").validate({
        ...
        messages: {
            carline_category_name: "Nama carline tidak boleh kosong",
        },
        errorElement: "em",
